...
#include <OPEN_ROBOT_PIC18F4520_RECOM_40MHz.h>
...
short posL_reached, posR_reached;      // we have reached the left or right position.
...
short leftMotorStalled;                // Has either the left or right motor stalled during a position move?
short rightMotorStalled;               // Has either the left or right motor stalled during a position move?
int leftMotorStallFactor;              // Maintain stall factors to force motor into position;
int rightMotorStallFactor;             // Maintain stall factors to force motor into position;

// constants
#define POS_INTEG_LIMIT 40             // maximum velocity value that we allow the position integral term to grow to to prevent windup
#define INTEG_LIMIT 55                 // maximum error value that we allow the velocity integral term to grow to to prevent windup
#define MAX_SPEED 60                   // maximum velocity in tenths of an inch per second (depends on the servo brand and other factors)
...
#define CORRECT_VELOCITY_PGAIN 1
#define CORRECT_VELOCITY TRUE

// Build Info.
#define BUILD_DATE "11/14/09"
#define BUILD_VERSION "5.1"
...
// eeprom[8]=left pwm motor offset, eeprom[9]=right pwm motor offset, eeprom[10]=position stop constant.
int eeprom[11];

int pic_restart_code;
...
#define HEART_BEAT_RATE 45
short beat_heart = FALSE;
...
/******************************************************************************/
/* This function will set the position stop constant.                         */
/******************************************************************************/
void set_position_stop_constant()
{
   eeprom[10]=get_rs232_int(2);
}
   ...
   if (kbhit()){
      ...
      if (rs232_buff[rs232_buff_index]==13){
         ...
         switch (rs232_buff[0]){
            ...
            case '!':
               printf("!Firmware Version = %S, Build Date = %S, Hardware Version = %S. Restart Code = %U\r", BUILD_VERSION, BUILD_DATE, HARDWARE_VERSION, pic_restart_code);
               break;
            ...
            case '@':
               ...
               // Get pwm offset.
               else if (rs232_buff[1] == 'G') printf("@%U,%U,%U\r",eeprom[8],eeprom[9],eeprom[10]);
               else if (rs232_buff[1] == 'S')
               {
                  set_position_stop_constant();
                  printf("@\r");
               }
               break;
         }
...
timer2_isr()
{
   ...
   if (timer2_seconds_counter == 25) // 25 = 20.479ms period (48.83 Hz)
   {
      servo_counter--;
      if (!servo_counter)
      {
      ...
      if (heart_beat_index > HEART_BEAT_RATE)
      {
         if (beat_heart)
         {
            output_high(PIN_C0);
...
void control_velocity()
{
   ...
   if (req_speed_R!=0)
   {
      out_R = (p_err_R + i_err_R - d_err_R + (req_speed_R)*12)/17;
      //out_R = (p_err_R + i_err_R - d_err_R + (req_speed_R)*12)/16;
      //printf("PID_out_R=%ld\r", out_R);
      if (out_R != 0)
      {
   ...
   if (req_speed_L!=0)
   {
      out_L = (p_err_L + i_err_L - d_err_L + (req_speed_L)*12)/17;
      //out_L = (p_err_L + i_err_L - d_err_L + (req_speed_L)*12)/16;
      //printf("PID_out_L=%ld\r", out_L);
      if (out_L != 0)
      {
   ...
   if (CORRECT_VELOCITY)
   {
      ...
      {
         if (abs(enc_speed_R)>abs(enc_speed_L)-1)
         {
            if (!bit_test(out_L, 15))
            {
               out_L += CORRECT_VELOCITY_PGAIN;
               out_R -= CORRECT_VELOCITY_PGAIN;
            }
            else
            {
               out_L-= CORRECT_VELOCITY_PGAIN;
         ...
         if (abs(enc_speed_L)>abs(enc_speed_R)-1)
         {
            if (!bit_test(out_R, 15))
            {
               out_R += CORRECT_VELOCITY_PGAIN;
   ...
   //printf("out_L=%ld , out_R=%ld, dir_L=%U, dir_R=%U : LftEnc%ld , RghtEnc%ld\r", out_L, out_R, dir_L, dir_R, enc_pos_L, enc_pos_R);
   //printf("out_L=%ld , out_R=%ld, dir_L=%U, dir_R=%U\r", out_L, out_R, dir_L, dir_R);
...
void set_position(signed int16 posR, signed int16 posL, signed long max_velocity)
{
   ...
   posL_reached=posR_reached=pos_reached = FALSE;
   ...
   //printf("ACCEL_INCREMENTS=%ld\r", ACCEL_INCREMENTS);
...
{
   return (signed int16)(((signed int32)req_pos_cur_max * value) / eeprom[10]);
}
...
void control_position()
{
   ...
   // Find out if either the left or right motor has stalled.
   if (prev_pos_err_L == req_pos_err_L) leftMotorStalled = TRUE;
   else leftMotorStalled=FALSE;
   if (prev_pos_err_R == req_pos_err_R) rightMotorStalled = TRUE;
   else rightMotorStalled=FALSE;

   if (leftMotorStalled) leftMotorStallFactor++;
   else leftMotorStallFactor=0;
   if (rightMotorStalled) rightMotorStallFactor++;
   else rightMotorStallFactor=0;
   ...
   // convert the total position error term to desired velocity.
   if (!posR_reached) spdR = calc_pos_speed(p_err_R + pos_i_err_R - d_err_R)/16;
   if (!posL_reached) spdL = calc_pos_speed(p_err_L + pos_i_err_L - d_err_L)/16;
  ...
  if (abs(req_pos_R) == abs(req_pos_L) && !posR_reached && !posL_reached)
   {
      if (abs(req_pos_L) > 99) spd_factor = 2;
      else spd_factor=1;
      ...
      else if (abs(enc_pos_R)>abs(enc_pos_L))
      {
   ...
   {
      // Right motor fwd.
      if (!bit_test(req_pos_R, 15))
      {
         // Right error positive.
         if (!bit_test(req_pos_err_R, 15))
         {
            if (req_pos_err_R < pos_tolerance)
            {
               posR_reached = TRUE;
            }
         }
         else posR_reached = TRUE;
      }
      else // Right motor backward.
      {
         // Right error positive.
         if (!bit_test(req_pos_err_R, 15)) posR_reached = TRUE;
         else
         {
            if (abs(req_pos_err_R) < pos_tolerance) posR_reached = TRUE;
         }
      }

      // Left motor fwd.
      if (!bit_test(req_pos_L, 15))
      {
         // Left error positive.
         if (!bit_test(req_pos_err_L, 15))
         {
            if (req_pos_err_L < pos_tolerance) posL_reached = TRUE;
         }
         else posL_reached = TRUE;
      }
      else // Left motor backward.
      {
         // Left error positive.
         if (!bit_test(req_pos_err_L, 15)) posL_reached = TRUE;
         else
         {
            if (abs(req_pos_err_L) < pos_tolerance) posL_reached = TRUE;
         }
      }

      if (posL_reached && posR_reached) pos_reached = TRUE;
      ...
      else req_pos_cur_max = req_pos_max_vel;    // we've reached the speed limit.
   }

   if (!pos_reached)
   {
      // Compensate for motor stall.
      if (leftMotorStalled)
      {
         if (!bit_test(spdL, 15)) spdL += leftMotorStallFactor;
         else spdL -= leftMotorStallFactor;
      }
      if (rightMotorStalled)
      {
         if (!bit_test(spdR, 15)) spdR += rightMotorStallFactor;
         else spdR -= rightMotorStallFactor;
      }
      if (posL_reached) spdL = 0;
      if (posR_reached) spdR = 0;
      //printf("spdL=%ld , spdR=%ld : LeftEnc%ld , RightEnc%ld\r", spdL, spdR, enc_pos_L, enc_pos_R);
      //printf("spdL=%ld , spdR=%ld\r", spdL, spdR);
...
main() {
   ...
   pic_restart_code = restart_cause();
...
void setup_hardware()
{
   ...
   for (i=0;i<11;i++) eeprom[i]=read_eeprom(i);

   // Set PID gains as needed.
   if (eeprom[0]==255) eeprom[0]=2;
   if (eeprom[1]==255) eeprom[1]=1;
   if (eeprom[2]==255) eeprom[2]=1;
   if (eeprom[3]==255) eeprom[3]=1;
   ...
   if (eeprom[8]==255) eeprom[8]=160;
   if (eeprom[9]==255) eeprom[9]=160;
   // Set position stop constant if eeprom[10]==255.
   if (eeprom[10]==255) eeprom[10]=80;

   leftMotorStalled=rightMotorStalled=FALSE;
   leftMotorStallFactor=rightMotorStallFactor=0;
   AD_Reflex=0;
   posL_reached=posR_reached=pos_reached=TRUE;
   reset_pid_variables();
   servo_counter = 5;
   sensor_id=1;                                 //Set a/d channel#1 to read first.
   battery_level=ad_chan_1=ad_chan_2=ad_chan_3=ad_chan_4=ad_chan_5=0;
   right_light=0;
   left_light=0;
   ...
   set_timer1(0);
   //setup_timer_2(T2_DIV_BY_1,255,4);            // 4883 Hz interrupt rate (204.8us period); 19531 Hz PWM frequency (For 20MHz Oscillator).
   setup_timer_2(T2_DIV_BY_1,255,8);            // 4883 Hz interrupt rate (204.8us period); 19531 Hz PWM frequency (For 40MHz Oscillator).
   ext_int_edge(0,H_TO_L);                        // set up RB0 to interrupt on falling edge of Left CLK
   init_motors();                               // Init. Motors.
   reset_pid_variables();                       // Reset PID Variables.
